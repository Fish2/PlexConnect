        ...
        try:
            ...
            if 'User-Agent' in self.headers and \
               'AppleTV' in self.headers['User-Agent']:
                ...
                # if enable_js_minify is enabled it will use min files
                dirname = path.dirname(self.path)
                basename = path.basename(self.path)
                if g_param['CSettings'].getSetting('enable_js_minify')=='False':
                    if basename in ("application.js", "main.js", "javascript-packed.js", "bootstrap.js") or basename.endswith(".js") and dirname == '/js':
                    ...
                    if basename in ("application.min.js", "main.min.js", "javascript-packed.min.js", "bootstrap.min.js") or basename.endswith(".min.js") and dirname == '/js':
                        if basename in ("main.min.js", "javascript-packed.min.js", "bootstrap.min.js"):
                            basename = "application.min.js"
                    dprint(__name__, 1, "serving /js/{0}", basename)
                    JS = JSConverter(basename, options)
                    self.sendResponse(JS, 'text/javascript', True)
                    return
